...
`test_results.png`保存了运行成功截图

使用方式：
``` sh
python TestHarness.py -s Sender.py -r Receiver.py
# 可用-t指定超时时间，不指定则默认为0.5，例如-t 0.05(建议使用小的timeout)
# 可用-f指定文件，不指定则默认为README.md，例如-f test.png
```
...
Sender.py
TestHarness.py
tests/OutOfOrderTest.py
tests/RepeatedArrivalTest.py
DataErrorTest.py
SackDataErrorTest.py
```
